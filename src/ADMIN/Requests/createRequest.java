...
public class createRequest extends javax.swing.JFrame {
    ...
    private void loadCitizens() {
        try {
            ...
            try (Connection con = db.getConnection()) {
                // fname and lname are from users table, join with citizens
                String sql = "SELECT c.citizen_id, u.fname, u.lname FROM citizens c JOIN users u ON c.U_Id = u.U_Id";
                java.sql.PreparedStatement pst = con.prepareStatement(sql);
                java.sql.ResultSet rs = pst.executeQuery();
                citizencombobox.removeAllItems();
                citizencombobox.addItem("Select Citizen");
                while (rs.next()) {
                    int citizenId = rs.getInt("citizen_id");
                    String fname = rs.getString("fname");
                    String lname = rs.getString("lname");
                    citizencombobox.addItem(citizenId + " - " + fname + " " + lname);
                }
            }
        } catch (Exception e) {
    ...
    private void initComponents() {
        ...
        citizencombobox = new javax.swing.JComboBox<>();
        ...
        citizencombobox.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Select Citizen" }));
        jPanel2.add(citizencombobox, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 50, 180, 30));
    ...
    private void requestDocumentMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_requestDocumentMouseClicked
        String selectedCitizen = (String) citizencombobox.getSelectedItem();
        if (selectedCitizen == null || selectedCitizen.equals("Select Citizen")) {
            JOptionPane.showMessageDialog(this, "Please select a citizen.");
            return;
        }
        int citizenId;
        try {
            String[] parts = selectedCitizen.split(" - ", 2);
            citizenId = Integer.parseInt(parts[0]);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Invalid citizen selection.");
            return;
        }
    ...
    private javax.swing.JComboBox<String> citizencombobox;
